+++
title = "Mad God's Overhaul"
type = "home"
+++

<h1><a href="/images/tes5-mgo@2x.webp"><img src="/images/tes5-mgo.webp" srcset="/mgo-tutorial/images/tes5-mgo@2x.webp 2x" alt="Mad God's Overhaul for Skyrim VR"></a></h1>

When you think of going on an adventure, what comes to mind? Constantly pausing the action and digging through menus? Lots and lots of menus? So you can replace your floating, disembodied hand with a sword? (I mean, do you want a hand, or do you want a sword? Pick one. Don’t get greedy.)

{{< video name="equip-vanilla" border="border" >}}
Hand or sword: pick one.
{{< /video >}}

And suppose you want to pick up a potion off of a shelf. To feel like you’re really there, you’d want to point in its general direction and press `A`, right? Just like real life!

## There's a better way

But seriously, folks, if that all sounds immersion-breaking and maybe even a bit tedious, I have good news.

Meet Mad God’s Overhaul, a mod list for _The Elder Scrolls V: Skyrim VR_.

This guide will walk you through setting up MGO, from download and installation to configuration, and even how the mod list changes some of Skyrim VR's gameplay systems.

{{< video name="equip-mgo" border="border" >}}
Now you can have a sword _and_ hands!
{{< /video >}}

MGO includes somewhere in the neighborhood of 1,600 mods. You don’t need to be familiar with all of them to enjoy yourself, and the old, less-immersive ways of doing things are (mostly) still there, so you don’t have to memorize all the new stuff at once—or ever. But if you don’t even know what’s possible, you won’t know where to start to make the gameplay more immersive and, dare I say, more fun.

<div class="next">
  <a href="/start">Let's go.  <i class="fa fa-circle-right"></i></a>
</div>